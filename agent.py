...
class PEAgent(BaseModel):
    ...
    def add_long_memory(self, memory_content: str, order: bool) -> List[str]:
        """Add an observation or memory to the agent's memory."""
        if order == False:
            self.memory[0].append(memory_content)
        else:
            self.memory[1].append(memory_content)
        return  self.memory
    ...
    def get_self_belief(self, observation: str, recipient_name: str, short_memory_summary:str, pattern:str = "", mode: str = "second_tom", oppo_pattern:str = "",oppo_belief:str = "", last_k:int=5) -> str:
        ...
        if self.order == "first_hand":
            long_memory = self.memory[0][-last_k:]
        else:
            long_memory = self.memory[1][-last_k:]
